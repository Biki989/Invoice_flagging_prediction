...
async function predictInvoice(event) {
  ...
  try {
    ...
    showInvoiceResult(resultEl, pred.Predicted_Flag, pred.rule_flag, pred.ml_flag, pred.reason);
...
function showInvoiceResult(el, flag, ruleFlag, mlFlag, reason) {
  ...
  if (flag === 0) {
    ...
    // Build a descriptive note based on what triggered the flag
    let note = "";
    if (ruleFlag && mlFlag) {
      note = `Rule violation: ${reason}. The ML model also independently flagged this invoice.`;
    } else if (ruleFlag) {
      note = `Rule violation: ${reason}. Auto-flagged before ML analysis.`;
    } else {
      note = "Anomaly detected. This invoice deviates significantly from expected patterns and requires manual audit review.";
    }
